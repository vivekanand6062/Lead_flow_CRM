...
import { Plus, AlertTriangle, Clock, Calendar, CheckCircle2 } from 'lucide-react';
...
export const SalesFollowUpsPage: React.FC = () => {
  ...
  const fetchFollowUps = async () => {
    try {
      ...
      console.error('Failed to load my follow-ups', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-text-muted-light dark:text-text-muted-dark mt-0.5">
            Prioritized tasks, scheduled call times, and customer commitment milestones.
          </p>
        </div>
      ...
      {loading ? (
        <div className="p-8 text-center text-text-muted-light dark:text-text-muted-dark">
          ...
          <span className="text-xs">Loading my follow-ups...</span>
        </div>
      ) : (
            ...
            <div className="space-y-3">
              ...
              {categorized.today.length === 0 ? (
                <p className="text-xs text-text-muted-light dark:text-text-muted-dark p-4 border border-dashed border-border-light dark:border-border-dark rounded-xl text-center">
                  No tasks scheduled for today. Good time for proactive prospect outreach!
                </p>
              ) : (
                <div className="space-y-2.5">
